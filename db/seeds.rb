...
if Claim.all.count == 0
 ...
 from_shp_sql = `shp2pgsql -c -g geom -t '2D' -W 'latin1' -s 3005 #{Rails.root.join('db', 'shpfiles', 'MTA_ACQ_TE_polygon.shp')} claims_ref`
  ...
  connection.execute <<-SQL
          ...
          insert into claims(gid, tnrtpcd, ttltpdsc, tnrsbtpcd, prcntowner, ntrtmstmp, tnrsbtpdsc, owner_name, trmntntpds, tag_number, objectid, gdtdt, ntrsrd, claim_name, num_owners, clientnum, issue_date, protected, pdtsrd, pdttmstmp, tnrtpdscrp, tnrnmbrd, rvsnnmbr, fcode, trmntndt, rnhctrs, ttltpcd, geom) 
                      select gid, tnrtpcd, ttltpdsc, tnrsbtpcd, prcntowner, ntrtmstmp, tnrsbtpdsc, owner_name, trmntntpds, tag_number, objectid, gdtdt, ntrsrd, claim_name, num_owners, clientnum, issue_date, protected, pdtsrd, pdttmstmp, tnrtpdscrp, tnrnmbrd, rvsnnmbr, fcode, trmntndt, rnhctrs, ttltpcd, ST_Transform(geom,4326) AS geom  from claims_ref;
      SQL
